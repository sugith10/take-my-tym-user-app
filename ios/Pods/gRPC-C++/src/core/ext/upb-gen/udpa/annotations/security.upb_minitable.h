/* This file was generated by upb_generator from the input file:
 *
 *     udpa/annotations/security.proto
 *
 * Do not edit -- your changes will be discarded when the file is
 * regenerated. */

#ifndef UDPA_ANNOTATIONS_SECURITY_PROTO_UPB_MINITABLE_H_
#define UDPA_ANNOTATIONS_SECURITY_PROTO_UPB_MINITABLE_H_

#include "upb/generated_code_support.h"

// Must be last.
#include "upb/port/def.inc"

#ifdef __cplusplus
extern "C" {
#endif

extern const upb_MiniTable udpa__annotations__FieldSecurityAnnotation_msg_init;
extern const upb_MiniTableExtension udpa_annotations_security_ext;

extern const upb_MiniTableFile udpa_annotations_security_proto_upb_file_layout;

#ifdef __cplusplus
}  /* extern "C" */
#endif

#include "upb/port/undef.inc"

#endif  /* UDPA_ANNOTATIONS_SECURITY_PROTO_UPB_MINITABLE_H_ */
